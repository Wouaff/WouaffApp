...
export async function sendContactEmail(from: string, subject: string, text: string): Promise<boolean> {
  ...
  try {
    await transporter.sendMail({
      ...
      to: CONTACT_EMAIL,
      replyTo: from,
      subject: `[Contact Wouaff] ${subject || 'Message'}`,
      text,
    });
    lastError = null;
    return true;
    ...
    console.error('[EMAIL] sendContact error:', (err as { message?: string }).message);
    return false;
  }
}
