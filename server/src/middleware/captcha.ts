...
import { isCaptchaRequired, verifyTurnstileToken } from '../config/captcha.js';
...
async function checkCaptcha(req: Request, res: Response, next: NextFunction, force: boolean): Promise<void> {
  if (!force && !isCaptchaRequired()) return next();
...
export function verifyCaptcha(req: Request, res: Response, next: NextFunction): void {
  void checkCaptcha(req, res, next, false).catch(() => {
...
export function verifyCaptchaStrict(req: Request, res: Response, next: NextFunction): void {
  void checkCaptcha(req, res, next, true).catch(() => {
    res.status(503).json({ error: 'Service de vérification indisponible' });
  });
}
...
export function verifyCaptchaIfNewAccount(req: Request, res: Response, next: NextFunction): void {
  void (async () => {
    try {
      ...
      return await checkCaptcha(req, res, next, false);
    } catch {
      res.status(503).json({ error: 'Service de vérification indisponible' });
    }
  })();
